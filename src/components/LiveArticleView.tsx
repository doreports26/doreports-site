...
import { transformSanityDocToArticle, type ArticleItem } from '@/lib/api'

interface LiveArticleViewProps {
...
export function LiveArticleView({
  ...
  useEffect(() => {
    ...
    // 1. Notify parent frame (Sanity Studio PreviewPane) that preview iframe is ready
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: 'DOREPORTS_PREVIEW_READY', slug }, '*')
    }

    // 2. Listen for live draft updates from Sanity Studio
    function handleMessage(event: MessageEvent) {
      // Validate event structure
      if (
        event.data &&
        event.data.type === 'DOREPORTS_DRAFT_UPDATE' &&
        event.data.document
      ) {
        try {
          ...
          console.error('[LiveArticleView] Error updating live preview:', err)
        }
      }
    }
  ...
  const activeArticle: ArticleItem = article || {
    ...
    tag: 'Do Reports',
    tags: ['Do Reports', 'Draft'],
    author: 'Do Reports Desk',
    snippet: 'Sanity Studio मध्ये लेख संपादित करा. तुमचा मजकूर येथे रिअल-टाइम लाईव्ह दिसेल.',
    content: null,
    rawContent: '',
    category: { name: 'ताज्या घडामोडी', slug: 'latest-news' },
  }

  const author = activeArticle.authorDetails || {
    name: activeArticle.author || 'Do Reports Desk',
    fullName: activeArticle.author || 'Do Reports Desk',
    role: 'Special Correspondent | Do Reports',
    avatar: null,
    avatarLetter: 'DR',
    verified: true,
    bio: 'Do Reports न्यूज डेस्कवरील अनुभवी पत्रकार. चालू घडामोडी आणि विशेष वृत्तांकनाचे काम पाहतात.',
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-10 items-start relative">
          ...
          <div className="lg:col-span-8 flex flex-col">
            ...
            <h1 className="text-[26px] sm:text-[32px] md:text-[38px] font-bold text-[#090909] leading-tight md:leading-[1.3] break-words mb-5 md:mb-6">
              {activeArticle.title || 'शीर्षक प्रविष्ट करा...'}
            </h1>
            ...
            {/* Content (Article Body - RichText / PortableText / Plaintext Support) */}
            <div className="max-w-none mb-10">
              <RichTextRenderer
                content={activeArticle.content}
                rawContent={activeArticle.rawContent}
                snippet={activeArticle.snippet}
              />
            </div>
            ...
            <div className="border border-gray-200 bg-gray-50/50 p-5 sm:p-6 flex flex-col sm:flex-row items-center sm:items-start gap-5 mb-10 rounded-xl">
              ...
              <div className="flex flex-col items-center sm:items-start text-center sm:text-left">
                ...
                <p className="text-gray-600 text-sm leading-relaxed">
                  {author.bio ||
                    'Do Reports न्यूज डेस्कवरील अनुभवी पत्रकार. चालू घडामोडी आणि विशेष वृत्तांकनाचे काम पाहतात.'}
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3 text-gray-800">
                ...
                <div>
                  <h4 className="font-bold text-[15px] text-gray-900">
                    Join Do Reports WhatsApp Channel
                  </h4>
            ...
            {/* Latest Stories Grid */}
            {latestStories.filter((s) => s.slug !== activeArticle.slug).length > 0 && (
              <div className="flex flex-col mb-8 pt-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                  {latestStories
                    .filter((s) => s.slug !== activeArticle.slug)
                    .slice(0, 3)
                    .map((story, idx) => (
