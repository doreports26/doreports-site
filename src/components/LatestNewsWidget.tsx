      ...
      <Link href={`/article/${mainStory.slug}`}>
        <div className="group cursor-pointer mb-6 border-b border-gray-100 pb-6">
          <div className="relative w-full h-[220px] bg-gray-200 overflow-hidden shadow-sm">
            {/* Tag */}
            <div className="absolute top-0 right-0 bg-[#cd0442] text-white text-xs font-bold px-3 py-1 z-20">
              {mainStory.tag || "Do Reports"}
            </div>
          ...
          <Link href={`/article/${news.slug}`} key={news.slug}>
            ...
            >
              ...
              <div className="flex-1 pr-4 flex flex-col justify-between">
                <h4 className={`text-base font-bold leading-tight mb-3 line-clamp-3 transition-colors ${news.highlight ? 'text-[#cd0442]' : 'text-[#090909] group-hover:text-[#cd0442]'}`}>
                  {news.title}
                </h4>
              ...
              <div className="w-[100px] h-[70px] bg-gray-200 shrink-0 relative overflow-hidden rounded-[2px]">
                ...
                />
                {news.tag && (
                  <div className="absolute top-0 right-0 bg-[#090909] text-white text-[9px] font-bold px-1.5 py-0.5 z-10">
                    {news.tag}
                  </div>
                )}
                {/* Image hover effect */}
